...
class DataBaseSampler(object):
    ...
    def sample_all(self, root_path, gt_boxes, gt_names, num_point_features=4,
                   road_planes=None, calib=None,sample_idx=None,put_same_ground=True):
        sampled_num_dict = {}
        ...
        for class_name, sampled_num in zip(self.sample_classes, sample_num_per_class):
            if sampled_num > 0:
                ...
                if len(sampled_cls) > 0:
                    sampled_name.append(class_name)
        ...
        # if sample_idx !=None:
        #     os.makedirs(os.path.join(root_path,"augmentation"),exist_ok=True)
        #     aug_gt_boxes_file = os.path.join(root_path,"augmentation","%s.txt"%sample_idx)
        #     f_aug = open(aug_gt_boxes_file,"w")
        #     for same_class_boxes,aug_name in zip(sampled_gt_boxes,sampled_name):
        #         for aug_box in same_class_boxes:
        #             aug_box = list(aug_box)
        #             lanes = ""
        #             for pose_id in range(len(aug_box)):
        #                 if pose_id==0:
        #                     lanes += aug_name
        #                     lanes += " "
        #                 lanes += str(aug_box[pose_id])
        #                 lanes += " "
        #             lanes += "\n"
        #             f_aug.writelines(lanes)
        #     f_aug.close()
        ret = None
        if len(sampled) > 0:
            ...
            if put_same_ground and road_planes is None:
                mean_ground = np.mean(center[:,2])
                # mean_ground = -1.81
                mv_height = sampled_gt_boxes[:,2] - mean_ground
            ...
            ret = {'gt_names': np.array([s['name'] for s in sampled]),
                   ...
                   'group_ids': np.arange(len(gt_boxes), len(gt_boxes) + len(sampled))}
    ...
    def sample_class_v2(self, name, num, gt_boxes):
        ...
        if num_gt>0:
            gt_boxes_bv = anno_utils.center2corner_leishen(gt_boxes, return_temp_corner=False)[:, 0:4, 0:2]  # (N, 4, 2)
            boxes = np.concatenate([gt_boxes, sp_boxes], axis=0).copy()
            sp_boxes_new = boxes[gt_boxes.shape[0]:]
            sp_boxes_bv = anno_utils.center2corner_leishen(sp_boxes_new, return_temp_corner=False)[:, 0:4,0:2]  # (N, 4, 2)
