...
__device__ inline int check_pt_in_box3d(const float *pt, const float *box3d, float &local_x, float &local_y){
    ...
    //float in_flag = (local_x > -l / 2.0) & (local_x < l / 2.0) & (local_y > -w / 2.0) & (local_y < w / 2.0);//kitti
    float in_flag = (local_x > -w / 2.0) & (local_x < w / 2.0) & (local_y > -l / 2.0) & (local_y < l / 2.0); //leishen
    return in_flag;
}
