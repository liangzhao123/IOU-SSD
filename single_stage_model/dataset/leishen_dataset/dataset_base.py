...
# from single_stage_model.roiaware_pool3d import roiaware_pool3d_utils
...
class LeiShenTemplate(Dataset):
    ...
    def get_lidar(self, idx):
        lidar_file = os.path.join(self.root_path, 'bin', '%s.bin' % idx)
        assert os.path.exists(lidar_file)
        return np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)

    def get_image_shape(self, idx):
        img_file = os.path.join(self.root_path, 'image_2', '%s.png' % idx)
        assert os.path.exists(img_file)
        return np.array(io.imread(img_file).shape[:2], dtype=np.int32)

    def get_img(self,idx):
        image_file = os.path.join(self.root_path,"image_2","%s.png"%idx)
    ...
    def get_label(self, idx):
        label_file = os.path.join(self.root_path, "annotation",'%s.txt' % idx)
        assert os.path.exists(label_file)
        return anno_utils.get_objects_from_label(label_file)

    def get_road_plane(self, idx):
        plane_file = os.path.join(self.root_path, 'planes', '000000.txt')
    ...
    def get_info(self,sample_idx,has_label=True,count_inside_pts=True):
        ...
        if has_label:
            ...
            annotations['truncated'] = None
            annotations['occluded'] = None
            annotations['alpha'] = None
            annotations['bbox'] = None
            annotations['dimensions'] = np.array([[obj.l, obj.h, obj.w] for obj in obj_list])  # lhw(camera) format
            annotations['location'] = np.concatenate([obj.loc.reshape(1, 3) for obj in obj_list], axis=0)
            annotations['rotation_y'] = np.array([obj.ry for obj in obj_list])#leishen are inverse to kitti
            annotations['score'] = None
            annotations['difficulty'] = None
            ...
            # loc_lidar = calib.rect_to_lidar(loc) #the frame of leishen lidar annotation is lidar frame not camera frame that like in kitti
            ...
            if count_inside_pts:
                ...
                corners_lidar = anno_utils.center2corner_leishen(gt_boxes_lidar)
                num_points_in_gt = -np.ones(num_gt, dtype=np.int32)

                src_code = False
                if src_code:
                    for k in range(num_objects):
                        flag = anno_utils.in_hull(points[:, 0:3], corners_lidar[k])
                        num_points_in_gt[k] = flag.sum()
                        if num_points_in_gt[k]==0:
                            print("no points:",sample_idx)
                else:
                    flags = leishen_opts_utils.points_in_boxes_cpu(torch.from_numpy(points[:, 0:3]), torch.from_numpy(gt_boxes_lidar)).numpy()
                    for k in range(num_objects):
                        num_points_in_gt[k] = (flags[k]>0).sum()
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if count_inside_pts:
                    ...
                    for k in range(num_objects):
                        flag = anno_utils.in_hull(pts_fov[:, 0:3], corners_lidar[k])
                        num_points_in_gt[k] = flag.sum()
                    annotations['num_points_in_gt'] = num_points_in_gt
