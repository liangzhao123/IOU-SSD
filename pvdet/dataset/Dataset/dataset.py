...
class KittiTemplate(Dataset):
    ...
    def get_info(self,sample_idx,has_label=True,count_inside_pts=True):
        ...
        img_info = {"image_id":sample_idx,"image_shape":self.get_image_shape(sample_idx)}
        info["image"]=img_info
        ...
        if has_label:
            ...
            if count_inside_pts:
                ...
                pts_rect_fov_flag = self.get_fov_flag(pts_rect,self.get_image_shape(sample_idx),calib)
                pts_fov = points[pts_rect_fov_flag]

                corners_lidar = box_utils.boxes3d_to_corners3d_lidar(gt_boxes_lidar)#[N,8,3]雷达坐标系的corner
                num_pts_in_gt = -np.ones((num_gt),dtype=np.int32)

                for k in range(num_object):
                    flag = box_utils.in_hull(pts_fov[:,:3],corners_lidar[k])
                    num_pts_in_gt[k] = flag.sum()
                annotations["num_points_in_gt"] = num_pts_in_gt
        return info
...
class KittiDataset(KittiTemplate):
    def __init__(self,datapath=cfg.DATA_DIR,class_name=cfg.CLASS_NAMES,training=True,split="train",logger=None,args=None):
    ...
    def include_kitti_data(self,mode,logger,args):
        ...
        if self.split=="train":
            ...
            print("Not Valid split,please check split:(only support :train,val)")
