from new_train.utils import box_code_utils
from new_train.model.rpn.assign_target import AxisAlignedTargetAssigner
...
class Cov2dBase(nn.Module):
    def __init__(self, grid_size):
        super().__init__()

        self.class_names = cfg.CLASS_NAMES
        self.num_class = len(cfg.CLASS_NAMES)
        self.box_coder = box_code_utils.ResidualCoder_v1()
        self.box_code_size = self.box_coder.code_size
        target_cfg = cfg.MODEL.RPN["target_config"]
        ...
        anchors, self.num_anchors_per_location = self.get_anchor(grid_size,
                                                                 target_cfg.ANCHOR_GENERATOR)
        self.anchors = [x.cuda() for x in anchors]
        self.num_anchors_per_location = sum(self.num_anchors_per_location)
        self.build_losses(cfg.MODEL.RPN["loss"])
    ...
    def get_reg_loss(self,):
        ...
        reg_loss = reg_loss*cfg.MODEL.RPN.loss["reg_loss_weight"]
    ...
    def get_cls_loss(self):
        ...
        cls_loss = cls_loss * cfg.MODEL.RPN.loss["cls_weight"]
    ...
    def get_dir_loss(self):
        ...
        if self.forward_ret_dict['dir_preds'] is not None:
            dir_target = self.get_dir_target(cfg.MODEL.RPN.conv_2d_config["dir_offset"],
                                             cfg.MODEL.RPN.conv_2d_config["num_direction_bins"])
        dir_preds = self.forward_ret_dict['dir_preds'].reshape(batch_size,-1,cfg.MODEL.RPN.conv_2d_config["num_direction_bins"])
        dir_loss = self.dir_loss_layer(dir_preds,dir_target,dir_weigts)
        dir_loss = dir_loss.sum()/batch_size
        dir_loss = dir_loss * cfg.MODEL.RPN.loss["dir_loss_weight"]
    ...
    def predict_box(self):
        ...
        #角度处理
        num_dir_bins = cfg.MODEL.RPN.conv_2d_config["num_direction_bins"]
        dir_offset = cfg.MODEL.RPN.conv_2d_config["dir_offset"]
        dir_limit_offset = cfg.MODEL.RPN.conv_2d_config["dir_limit_offset"]
...
from new_train.utils.pytorch_utils import Empty, Sequential


class Conv2dNet(Cov2dBase):
    def __init__(self, grid_size):
        super().__init__(grid_size)
        self.config = cfg.MODEL.RPN["conv_2d_config"]
        self._concat_input = False
        ...
        in_filters = [self.config['num_input_features'], *self.config['num_filters'][:-1]]
        blocks = []
        deblocks = []
        for i, layer_num in enumerate(self.config['layer_nums']):
            block = Sequential(
                nn.ZeroPad2d(1),
                Conv2d(in_filters[i], self.config['num_filters'][i], 3, stride=self.config['layer_strides'][i]),
                BatchNorm2d(self.config['num_filters'][i]),
                nn.ReLU(),
            )
            for j in range(layer_num):
                block.add(Conv2d(self.config['num_filters'][i], self.config['num_filters'][i], 3, padding=1))
                block.add(BatchNorm2d(self.config['num_filters'][i]))
                block.add(nn.ReLU())
            blocks.append(block)
            deblock = Sequential(
                ConvTranspose2d(
                    self.config['num_filters'][i], self.config['num_upsample_filters'][i],
                    self.config['upsample_strides'][i],
                    stride=self.config['upsample_strides'][i]
                ),
                BatchNorm2d(self.config['num_upsample_filters'][i]),
                nn.ReLU(),
            )
            deblocks.append(deblock)
        c_in = sum(self.config["num_upsample_filters"])
        if self._concat_input:
            c_in += self.config["num_input_features"]
        self.blocks = nn.ModuleList(blocks)
        self.deblocks = nn.ModuleList(deblocks)
        if self.config["encode_background_as_zeros"]:
        ...
        reg_channel = self.num_anchors_per_location * self.box_code_size
        self.conv_box = nn.Conv2d(c_in,reg_channel,1)
        if self.config["use_direction_classifier"]:
            self.conv_dir_cls = nn.Conv2d(c_in,self.num_anchors_per_location*self.config["num_direction_bins"],1)
        self.init_weights()
    ...
    def forward(self, x_in,gt_boxes):
        ...
        if self.training:
            ...
            iou3d = iou3d_nms_utils.boxes_iou3d_gpu(box_gt_recover[0],gt_boxes[0][...,:7])
            idx = (iou3d > 0.98).int()
